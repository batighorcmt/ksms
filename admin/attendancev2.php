...
// Get classes and sections based on user role
if ($user_role == 'super_admin') {
    ...
    $stmt = $pdo->prepare("
        SELECT c.* 
        FROM classes c
        INNER JOIN sections ta ON c.id = ta.class_id 
        WHERE ta.teacher_id = ? AND c.status='active'
...
$is_authorized = true; // Assume authorized unless check fails
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['mark_attendance'])) {
    ...
    if ($user_role == 'teacher') {
        ...
        ");
        $auth_params = [$user_id, $class_id, $section_id];
        $auth_stmt->execute($auth_params);
        $auth_result = $auth_stmt->fetch();
        
        if ($auth_result['count'] == 0) {
            $_SESSION['error'] = "আপনি এই ক্লাস এবং শাখার জন্য অনুমোদিত নন।";
            $is_authorized = false;
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['view_attendance'])) {
    ...
    if ($user_role == 'teacher') {
        $auth_stmt = $pdo->prepare("
            ...
            AND (section_id IS NULL OR section_id = ?)
        ");
        $auth_params = [$user_id, $selected_class, $selected_section];
        $auth_stmt->execute($auth_params);
        $auth_result = $auth_stmt->fetch();
...
// Get sections based on selected class and user role
$sections = [];
if ($selected_class) {
    if ($user_role == 'super_admin') {
        ...
        // Teacher can only see sections they're assigned to for the selected class
        $section_stmt = $pdo->prepare("
            SELECT s.* 
            FROM sections s
            WHERE ta.teacher_id = ? AND s.class_id = ? AND s.status='active'
        ");
        $section_stmt->execute([$user_id, $selected_class]);
        $sections = $section_stmt->fetchAll();
    }
}
?>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card attendance-card">
                            ...
                            <div class="card-body">
                                <?php if (!$is_authorized && $user_role == 'teacher'): ?>
                                    <div class="alert alert-danger">
                                        <i class="fas fa-exclamation-triangle"></i> 
                                        আপনি এই ক্লাস এবং শাখার জন্য অনুমোদিত নন। শুধুমাত্র নির্ধারিত শিক্ষকগণই এই ক্লাসের উপস্থিতি রেকর্ড করতে পারেন।
                                    </div>
                                <?php else: ?>
